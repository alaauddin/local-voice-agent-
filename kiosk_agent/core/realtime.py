...
def build_realtime_session() -> tuple[dict, str]:
    ...
    try:
        ...
        instructions = (
            ...
            + "\n\n<realtime_behavior>\n"
            + "This is a native speech-to-speech session over OpenAI Realtime/WebRTC. The frontend "
            + "keeps ONE microphone MediaStream alive with echoCancellation, noiseSuppression, "
            + "autoGainControl and voiceIsolation (when supported) and uses server VAD for barge-in.\n"
            + "Listen continuously. Allow natural interruptions and stop your current response immediately "
            + "when the guest starts speaking (barge-in). Do not describe silence, audio processing, "
            + "transcription, or connection state.\n"
            + "Arabic quality: you are optimized for Gulf/Omani conversational Arabic and natural "
            + "Arabic-English code switching (e.g., WiFi, menu, AC, delivery). Understand Omani/Gulf "
            + "pronunciation, fillers (يعني، آه، همم، طيب), hesitations, self-corrections and incomplete "
            + "phrases. Use the transcript exactly as heard; do not normalize or replace the guest's "
            + "words destructively and preserve first and last words of each turn. If only part of the "
            + "audio is unclear, do not guess the whole turn — ask the guest to repeat only the unclear "
            + "part in one short question. Wait through normal thinking pauses; do not respond too early.\n"
            + "Keep ordinary spoken replies very short. Every audible reply must be entirely in Arabic. "
            + "Never answer in English, even when the guest uses English words. Translate internal service "
            + "names and tool results naturally into Arabic.\n"
